...
## 同步：ppdm v0.0.2
- **新增 `ppdm list`**：列出当前账号已发布的包（`name@version os/arch + url`）。
- **新增 `ppdm del <pkg> [version]`**：删除自己已发布的包，可只删某个版本。
- **后端新增 `web/ppdm/list.php`、`web/ppdm/del.php`**（按 `owner` 鉴权，他人包拒绝）。
- **ppdm 0.0.2 全量发布**：像 pmm 一样打 `.pdm` 进镜像（`dists/`+`files/`），
  可用 `pmm install ppdm` 安装；`install-ppdm.sh` 与 GitHub Release 资产同时更新。
- ppdm 版本从 `0.0.1` 升到 `0.0.2`（独立于 PMM 0.6.0）。

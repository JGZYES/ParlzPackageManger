...
static int g_parsing = 0;   /* re-entry guard while parsing a built-in table */
...
const char *pmm_tr(const char *key) {
    ...
    /* g_parsing guards against re-entry: json_parse -> fail() must never call
     * back into pmm_tr() while we are already parsing the built-in table. */
    if (!g_builtin && !g_parsing) {
        g_parsing = 1; g_builtin = json_parse(builtin_source()); g_parsing = 0;
    }
    if (g_builtin) {
        const char *b = json_str(g_builtin, key);
        if (b) return b;
    }
    if (!g_builtin_alt && !g_parsing) {
        g_parsing = 1;
